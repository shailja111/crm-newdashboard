...
function Leads() {
  ...
  //     axios
  //       .get(
  //         "https://1bb1-146-196-36-69.ngrok-free.app/api/LeadsGenerate/FetchAndSaveData"
  //       )
  //       .then((response) => {
  //         const { leadsList = [] } = response.data; // Destructure to get the leadsList from the response
  //         setAds(leadsList); // Set the leads data to state
  //         setLoading(false);
  //       })
  //       .catch((err) => {
  //         console.error("Error fetching the leads data:", err);
  //         setError(err);
  //         setLoading(false);
  //       });
  //   }, []);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //setAds(response.data.leadList);
        console.log(response.data.leadList);
        setLoading(false);
      } catch (err) {
        setError(err);
        setLoading(false);
  ...
  useEffect(() => {
    ...
    makeDroppable(
      ...
      moveLeadsFromDiscussionsToDecisionMaking
    );
    makeDroppable(
      ".contract-discussions-column",
      moveLeadsFromDiscussionsToContract
    );
    makeDroppable(
      ".contract-discussions-column",
      moveLeadsFromDecisionToContract
    );
    makeDroppable(
      ".discussions-column",
      moveLeadsFromDecisionMakingToDiscussions
    );
    makeDroppable(
      ".decision-making-column",
      moveLeadsFromContractToDecisionMaking
    );
  ...
  const moveLeadsToDiscussions = (ad, index) => {
    if (!discussions.includes(ad)) {
      // Check if ad is already in discussions
  ...
  const moveLeadsToDecisionMaking = (ad, index) => {
    if (!decisionMaking.includes(ad)) {
      // Check if ad is already in decision making
  ...
  const moveLeadsToContract = (ad, index) => {
    if (!contractDiscussions.includes(ad)) {
      // Check if ad is already in contract discussions
      ...
      console.log(
        "Lead already in contract discussions, skipping duplicate:",
        ad
      );
    }
  };
  ...
  const moveLeadsFromDiscussionsToDecisionMaking = (ad, index) => {
    if (!decisionMaking.includes(ad)) {
      ...
      setDiscussions((prevDiscussions) =>
        prevDiscussions.filter((_, i) => i !== index)
  ...
  const moveLeadsFromDecisionToContract = (ad, index) => {
    if (!contractDiscussions.includes(ad)) {
      // Check if ad is already in contract discussions
      console.log("Moving lead to contract discussions:", ad); // Debug log
      setDecisionMaking((prevDecisionMaking) =>
        prevDecisionMaking.filter((_, i) => i !== index)
  ...
  const moveLeadsFromContractToDecisionMaking = (ad, index) => {
    if (!decisionMaking.includes(ad)) {
      ...
      setContractDiscussions((prevContracts) =>
        prevContracts.filter((_, i) => i !== index)
      ); // Remove from contract discussions
      setDecisionMaking((prevDecisionMaking) => [...prevDecisionMaking, ad]); // Add to decision making
      removeLeadFromContract(ad); // If it's in contract, remove it
      removeLeadFromDiscussions(ad); // If it's in discussions, remove it
    } else {
  ...
  const removeLeadFromContract = (ad) => {
    setContractDiscussions((prevContracts) =>
      prevContracts.filter((contractAd) => contractAd !== ad)
    ); // Remove the ad if it exists in contract discussions
  };

  // Remove lead from decision making if it exists there
  const removeLeadFromDecisionMaking = (ad) => {
    setDecisionMaking((prevDecisionMaking) =>
      prevDecisionMaking.filter((decisionAd) => decisionAd !== ad)
    ); // Remove the ad if it exists in decision making
  };

  //Remove lead from discussions if it exists there
  const removeLeadFromDiscussions = (ad) => {
    setDiscussions((prevDiscussions) =>
      prevDiscussions.filter((discussionAd) => discussionAd !== ad)
    ); // Remove the ad if it exists in discussions
  };
    ...
    <div className="container-fluid mt-100">
      <div className="row">
        ...
        <div className="col-md-3 decision-making-column">
          ...
          <ul className="list-group">
            {decisionMaking.length > 0 ? (
              decisionMaking.map((ad, index) => (
                <li className="list-group-item mt-3" key={index}>
                  <LeadItems ad={ad} index={index} />
                </li>
              ))
